test_that("handle works", {
  expect_contains(
    curl:::handle_getheaders(get_curl_handle("text")),
    "Accept: application/text"
  )
  expect_contains(
    curl:::handle_getheaders(get_curl_handle("json")),
...
test_that("request accept works", {
  ...
  expect_error(
    get_request("https://httpbin.org/xml"),
...
test_that("verbose works", {
  ...
  expect_message(
    get_request("https://httpbin.org/json"),
...
test_that("check status works", {
  expect_error(
    get_request("https://httpbin.org/status/200", accept = "text"),
    regexp = NA
  )

  expect_error(
    get_request("https://httpbin.org/status/400"),
    regexp = "Status code: 400"
  )
})
