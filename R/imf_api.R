...
parse_urn <- function(urn) {
  ...
  list(agency = parts[[2L]], id = parts[[3L]], version = parts[[4L]])
}
...
find_concept <- function(concept_schemes, concept_id) {
  if (is.null(concept_schemes) || is.na(concept_id)) {
    return(NULL)
  }
  matches <- list()
  for (scheme in concept_schemes) {
    ...
    for (concept in concepts) {
      if (identical(concept$id, concept_id)) {
        matches[[length(matches) + 1L]] <- concept
      }
    }
  }
  if (!length(matches)) {
  ...
  has_enumeration <- vapply(
    matches,
    function(concept) {
      !is.null(concept$coreRepresentation$enumeration)
    },
    logical(1)
  )
  if (any(has_enumeration)) {
    return(matches[[which(has_enumeration)[[1L]]]])
  }
  matches[[1L]]
}
...
dimension_records <- function(resolved) {
  ...
  records <- lapply(dimensions, function(dimension) {
    concept_id <- extract_concept_id(dimension$conceptIdentity)
    concept <- find_concept(resolved$concepts, concept_id)
    ...
    list(
      ...
      name = value_or_na(first_existing(concept, c("name", "id"))),
...
dimension_value <- function(dimension, index) {
  ...
  if (is.null(values) || position < 1L || position > length(values)) {
    return(NA_character_)
  }
...
empty_data_result <- function(structure, attributes) {
  ...
  dimension_names <- vapply(
    c(series_dimensions, observation_dimensions),
    function(dimension) value_or_na(dimension$id),
    character(1)
  )
  columns <- rep("character", length(dimension_names))
  names(columns) <- dimension_names
  columns <- c(columns, OBS_VALUE = "numeric")
...
parse_data_message <- function(response, attributes = character()) {
  if (
    is.null(response) ||
      is.null(response$data$dataSets) ||
      !length(response$data$dataSets)
  ) {
    return(empty_tibble(c(TIME_PERIOD = "character", OBS_VALUE = "numeric")))
  }
  rows <- list()
  row_index <- 0L
  for (data_set in response$data$dataSets) {
    structure_index <- as.integer(data_set$structure) + 1L
    structure <- response$data$structures[[structure_index]]
    ...
    for (series_key in names(series)) {
      ...
      for (observation_key in names(observations)) {
        ...
        measure_values <- vapply(
          measures,
          function(value) {
            suppressWarnings(as.numeric(value_or_na(value)))
          },
          numeric(1)
        )
        ...
        attribute_values <- observation[-seq_len(measure_count)]
        ...
        selected_attributes <- all_attributes[attributes]
  ...
  if (!length(rows)) {
    structure <- response$data$structures[[1L]]
    return(empty_data_result(structure, attributes))
  }
  out <- do.call(
    rbind,
  ...
  measure_names <- vapply(
    response$data$structures[[1L]]$measures$observation,
    function(definition) value_or_na(definition$id),
    character(1)
  )
